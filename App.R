...
base_url <- "https://raw.githubusercontent.com/Morteza-Ab/Fingolimod/main/"

download_data <- function(url, filename) {
  if (!file.exists(filename)) {
    download.file(url, filename)
  }
  return(openxlsx::read.xlsx(filename))
}

# Downloading and reading the transcriptome data
datasets <- list(
  transcriptome = list(
    Eto_vs_CTRL = download_data(paste0(base_url, "clean_transcriptome_Eto_vs_CTRL.xlsx"), "clean_transcriptome_Eto_vs_CTRL.xlsx"),
    FTY_vs_CTRL = download_data(paste0(base_url, "clean_transcriptome_FTY_vs_CTRL.xlsx"), "clean_transcriptome_FTY_vs_CTRL.xlsx"),
    FTY_Eto_vs_CTRL = download_data(paste0(base_url, "clean_transcriptome_FTY_Eto_vs_CTRL.xlsx"), "clean_transcriptome_FTY_Eto_vs_CTRL.xlsx"),
    FTY_vs_Eto = download_data(paste0(base_url, "clean_transcriptome_FTY_vs_Eto.xlsx"), "clean_transcriptome_FTY_vs_Eto.xlsx"),
    FTY_Eto_vs_Eto = download_data(paste0(base_url, "clean_transcriptome_FTY_Eto_vs_Eto.xlsx"), "clean_transcriptome_FTY_Eto_vs_Eto.xlsx"),
    FTY_Eto_vs_FTY = download_data(paste0(base_url, "clean_transcriptome_FTY_Eto_vs_FTY.xlsx"), "clean_transcriptome_FTY_Eto_vs_FTY.xlsx")
  ),
  
  # Downloading and reading the proteome data
  proteome = list(
    Eto_vs_CTRL = download_data(paste0(base_url, "set1.xlsx"), "set1.xlsx"),
    FTY_vs_CTRL = download_data(paste0(base_url, "set2.xlsx"), "set2.xlsx"),
    FTY_Eto_vs_CTRL = download_data(paste0(base_url, "set3.xlsx"), "set3.xlsx"),
    FTY_vs_Eto = download_data(paste0(base_url, "set4.xlsx"), "set4.xlsx"),
    FTY_Eto_vs_Eto = download_data(paste0(base_url, "set5.xlsx"), "set5.xlsx"),
    FTY_Eto_vs_FTY = download_data(paste0(base_url, "set6.xlsx"), "set6.xlsx")
  )
)
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      plotOutput("plot", width = "60%", height = "60%")
    )
  ),
  ...
  sidebarLayout(
    ...
    mainPanel(
      plotOutput("additional_plot", width = "60%", height = "600px")
    )
  )
)
...
server <- function(input, output, session) {
  output$dataset_selector <- renderUI({
    selectInput("dynamic_dataset", "Select Dataset", 
                choices = if (input$data_type == "Transcriptome") 
                  names(datasets$transcriptome) 
                else 
                  names(datasets$proteome))
  })
  
  output$plot <- renderPlot({
    req(input$data_type, input$dynamic_dataset, input$plot_type)
    
    df <- if (input$data_type == "Transcriptome") {
      datasets$transcriptome[[input$dynamic_dataset]]
    } else {
      datasets$proteome[[input$dynamic_dataset]]
    }
    ...
    dataset <- datasets[[tolower(input$dataset_type)]][[input$comparison]]
