...
# Define URLs and local file paths for transcriptome data
transcriptome_urls <- c(
  "https://github.com/Morteza-Ab/Shiny-Senescence/raw/main/clean_transcriptome_Eto_vs_CTRL.xlsx",
  "https://github.com/Morteza-Ab/Shiny-Senescence/raw/main/clean_transcriptome_FTY_vs_CTRL.xlsx",
  "https://github.com/Morteza-Ab/Shiny-Senescence/raw/main/clean_transcriptome_FTY_Eto_vs_CTRL.xlsx",
  "https://github.com/Morteza-Ab/Shiny-Senescence/raw/main/clean_transcriptome_FTY_vs_Eto.xlsx",
  "https://github.com/Morteza-Ab/Shiny-Senescence/raw/main/clean_transcriptome_FTY_Eto_vs_Eto.xlsx",
  "https://github.com/Morteza-Ab/Shiny-Senescence/raw/main/clean_transcriptome_FTY_Eto_vs_FTY.xlsx"
)

transcriptome_files <- c(
  "clean_transcriptome_Eto_vs_CTRL.xlsx",
  "clean_transcriptome_FTY_vs_CTRL.xlsx",
  "clean_transcriptome_FTY_Eto_vs_CTRL.xlsx",
  "clean_transcriptome_FTY_vs_Eto.xlsx",
  "clean_transcriptome_FTY_Eto_vs_Eto.xlsx",
  "clean_transcriptome_FTY_Eto_vs_FTY.xlsx"
)

# Define URLs and local file paths for proteome data
proteome_urls <- c(
  "https://github.com/Morteza-Ab/Shiny-Senescence/raw/main/set1.xlsx",
  "https://github.com/Morteza-Ab/Shiny-Senescence/raw/main/set2.xlsx",
  "https://github.com/Morteza-Ab/Shiny-Senescence/raw/main/set3.xlsx",
  "https://github.com/Morteza-Ab/Shiny-Senescence/raw/main/set4.xlsx",
  "https://github.com/Morteza-Ab/Shiny-Senescence/raw/main/set5.xlsx",
  "https://github.com/Morteza-Ab/Shiny-Senescence/raw/main/set6.xlsx"
)
...
for (i in seq_along(transcriptome_urls)) {
  if (!file.exists(transcriptome_files[i])) {
    download.file(transcriptome_urls[i], destfile = transcriptome_files[i])
  }
}

# Download proteome files if they do not already exist
for (i in seq_along(proteome_urls)) {
  if (!file.exists(proteome_files[i])) {
    download.file(proteome_urls[i], destfile = proteome_files[i])
  }
}
...
datasets1 <- list(
  transcriptome1 = list(
    Eto_and_CTRL1 = transcriptome_Eto_and_CTRL1,
    FTY_and_CTRL1 = transcriptome_FTY_and_CTRL1,
    FTY_Eto_and_CTRL1 = transcriptome_FTY_Eto_and_CTRL1,
    FTY_and_Eto1 = transcriptome_FTY_and_Eto1,
    FTY_Eto_and_Eto1 = transcriptome_FTY_Eto_and_Eto1,
    FTY_Eto_and_FTY1 = transcriptome_FTY_Eto_and_FTY1
  ),
  proteome1 = list(
    Eto_and_CTRL1 = proteome_Eto_and_CTRL1,
    FTY_and_CTRL1 = proteome_FTY_and_CTRL1,
    FTY_Eto_and_CTRL1 = proteome_FTY_Eto_and_CTRL1,
    FTY_and_Eto1 = proteome_FTY_and_Eto1,
    FTY_Eto_and_Eto1 = proteome_FTY_Eto_and_Eto1,
    FTY_Eto_and_FTY1 = proteome_FTY_Eto_and_FTY1
  )
)
                          ...
                          fluidPage(
                            sidebarLayout(
                              sidebarPanel(
                                ...
                                selectInput("data_type_expr", "Select Data Type", choices = c("Transcriptome", "Proteome")),
                                selectInput("dataset_expr", "Select Dataset:", choices = c("Eto_vs_CTRL", "FTY_vs_CTRL", "FTY_Eto_vs_CTRL", "FTY_vs_Eto", "FTY_Eto_vs_Eto", "FTY_Eto_vs_FTY")),
                                #uiOutput("dataset_selector_expr"),
                            ...
                            sidebarLayout(
                              sidebarPanel(
                                ...
                                selectInput("dataset_type_enr", "Dataset Type:", choices = c("transcriptome", "proteome")),
                              ...
                              mainPanel(
                                plotOutput("additional_plot", width = "60%", height = "500px")
                              )
                            )
                          )
                 ),
                          ...
                          fluidPage(
                            sidebarLayout(
                              sidebarPanel(
                                ...
                                selectInput("data_type_feat", "Select Data Type", choices = c("Transcriptome1", "Proteome1")),
                                uiOutput("dataset_selector_feat"),
                                sliderInput("n_features", "Number of features", min = 10, max = 50, step = 5, value = 20)
                              ),
                              mainPanel(
                                plotOutput("heatmap_plot", width = "60%", height = "500px")
                              )
                            )
                          )
                 )
...
server <- function(input, output, session) {
  output$dataset_selector_expr <- renderUI({
    if (input$data_type_expr == "Transcriptome") {
      selectInput("dataset_expr", "Select Dataset", choices = names(datasets$transcriptome))
    } else {
      selectInput("dataset_expr", "Select Dataset", choices = names(datasets$proteome))
    }
  })
  
  output$plot <- renderPlot({
    req(input$data_type_expr, input$dataset_expr, input$plot_type)
    
    df <- if (input$data_type_expr == "Transcriptome") {
      datasets$transcriptome[[input$dataset_expr]]
    } else {
      datasets$proteome[[input$dataset_expr]]
    }
  ...
  output$additional_plot <- renderPlot({
    req(input$dataset_type_enr, input$comparison, input$regulation, input$enrichment)
    
    dataset <- datasets[[input$dataset_type_enr]][[input$comparison]]
  ...
  # Second Page - Feature Selection and Heatmap
  output$dataset_selector_feat <- renderUI({
    if (input$data_type_feat == "Transcriptome1") {
      selectInput("dataset_feat", "Select Dataset", choices = names(datasets1$transcriptome1))
    } else {
      selectInput("dataset_feat", "Select Dataset", choices = names(datasets1$proteome1))
    }
  })
  
  output$heatmap_plot <- renderPlot({
    req(input$data_type_feat, input$dataset_feat, input$n_features)
    df <- if (input$data_type_feat == "Transcriptome1") {
      datasets1$transcriptome1[[input$dataset_feat]]
    } else {
      datasets1$proteome1[[input$dataset_feat]]
    }
    selected_features <- bwf_feature_selection(df, input$n_features)
    # Plot code for heatmap goes here
  }, height = 800, res = 96)
}
